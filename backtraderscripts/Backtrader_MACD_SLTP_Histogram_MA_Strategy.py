...
from Backtrader_MACD_Histogram_MA_Indicator import MacdHistoThresholdIndicator

class BacktraderMacdSltpHistogramMaStrategy(bt.Strategy):
    ...
    def __init__(self, logger=None):
        ...
        self.macd = MacdHistoThresholdIndicator(self.data, fast=self.p.pfast, slow=self.p.pslow,
                                                signal=self.p.psignal)
    ...
    def next(self):
        ...
        self.logger.debug("Date:"+current_datetime.strftime('%Y-%m-%d %H:%M:%S')
                         ...
                         + "\t macd signal: " + str(self.macd.lines.macd_signal[0])
                         + "\t macd histo: " + str(self.macd.lines.macd_histo[0])
                         + "\t final signal: " + str(self.macd.lines.signal[0]))
        ...
        if not self.position:  # not in the market
            if self.macd.lines.signal[0] > 0:  # macd crosses signal to the upside
        ...
        elif self.macd.lines.signal[0] < 0:  # in the market & cross to the downside
            if self.position:
